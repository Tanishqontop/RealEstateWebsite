...
function setupPageTransitions() {
  const NUM_BLINDS = 10;                           // number of vertical blinds
  const blindsContainer = document.getElementById("blinds-container");
  const links = document.querySelectorAll("a");

  // 1) build the blinds elements
  ...
  // 2) attach click handler to each link
  links.forEach(link => {
    link.addEventListener("click", e => {
      ...
      // timeline: blinds drop down, then navigate
      gsap.timeline({
  ...
  // 3) on page-load reveal: blinds lift up
  gsap.fromTo(blinds,
    { scaleY: 1 },
    {
      scaleY: 0,
  ...
  // 4) fade/scale body for extra polish
  gsap.fromTo("body",
