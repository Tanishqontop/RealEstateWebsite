...
function setupButtonInteractions() {
    ...
    buttons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            // TODO: Replace the alert with the desired functionality.
            // Example options:
            // 1. Open a modal dialog to display more information or a form.
            // 2. Navigate to a specific section or page (e.g., window.location.href = '/join-us').
            // 3. Trigger an animation or interaction specific to the button's purpose.
        });
...
function animateImageReveal() {
    ...
    // Use clip-path to create a masking effect that reveals the image
    // Initially, the image is fully masked (hidden) with a clip-path polygon
    gsap.fromTo(imageSection, 
        { clipPath: 'polygon(0% 0%, 0% 0%, 0% 100%, 0% 100%)' }, 
        {
            // Gradually reveal the image by expanding the clip-path polygon
            clipPath: 'polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)',
            duration: 1.5,
            ease: 'power3.out',
            scrollTrigger: {
                // Trigger the animation when the '.image' section enters the viewport
                trigger: '.image',
                start: 'top 80%',
                toggleActions: 'play none none reverse'
            }
...
if (!Array.isArray(slides) || slides.some(slide => !slide.image || !slide.title)) {
    console.error("Invalid slides data. Ensure slides is an array with 'image' and 'title' properties.");
...
for(let i = 1; i<totalSlides; i++) {
    ...
    for(let j = 0; j < stripsCount; j++) {
       ...
       strip.src = slides[i].image;
       strip.alt = slides[i].title;
       strip.style.transform = "scale(1.25)";
...
function animateTitleChange(index, direction) {
    ...
    function getTitleIndexForProgress (imageProgress) {
        ...
        ScrollTrigger.create({
            ...
            // invalidateOnRefresh: true, // Removed to optimize performance
