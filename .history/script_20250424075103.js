...
function setupPageTransitions() {
  ...
  // 1) Inject CSS via JS (same as before)
  const css = `
    #blinds-container {
      position: fixed;
      top: 0; left: 0;
    ...
    .blind {
      flex: 1;
      background: #111; /* Change to your preferred color/gradient */
      transform: scaleY(0);
  ...
  // 2) Create blinds container and panels
  const blindsContainer = document.createElement('div');
  blindsContainer.id = 'blinds-container';
  for (let i = 0; i < NUM_BLINDS; i++) {
    const blind = document.createElement('div');
    blind.className = 'blind';
    blindsContainer.appendChild(blind);
  }
  document.body.appendChild(blindsContainer);
  const blinds = document.querySelectorAll('.blind');

  // 3) Animate on link click
  document.querySelectorAll('a').forEach(link => {
    link.addEventListener('click', e => {
      ...
      .to(blinds, {
        scaleY: 1,
        stagger: 0.15,        // Slow stagger for smooth entry
        duration: 1.2,        // Longer duration for smooth animation
        ease: 'power4.inOut'  // Smooth easing for more natural entry
      });

      // Optional: change background color during transition
      gsap.to('body', {
        backgroundColor: '#333', // Dark background during transition
        duration: 1.2,
        ease: 'power4.inOut'
      });
    });
  ...
  // 4) Animate blinds up on page load
  gsap.fromTo(blinds,
    { scaleY: 1 },
    {
      scaleY: 0,
      ...
      duration: 1.2,
      ease: 'power4.inOut',
      delay: 0.4  // Slight delay to keep the entrance smooth
    }
  );

  // 5) Smooth body fade-in
  gsap.fromTo('body',
    { opacity: 0, scale: 0.96 },
    { opacity: 1, scale: 1, duration: 1.2, ease: 'power2.out', delay: 0.7 }
  );
}
