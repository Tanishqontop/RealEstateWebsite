...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function animateTitleChange(index, direction) {
    if (index === currentTitleIndex || index < 0 || index >= travel.length) return;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const slideImages = document.querySelector('#img-T');
  const titleElement = document.querySelector('#title-text-1');
  const exploreLink = document.querySelector('.slide-link-1 a');
  ...
  const firstSlideImg = document.querySelector("#img-T img");
  firstSlideImg.style.transform = "scale(1.25)";
  ...
  function animateTitleChange(index, direction) {
    if (index === currentTitleIndex || index < 0 || index >= slides.length) {
      return;
    }
    ...
    onUpdate: function (self) {
      ...
      if (typeof imageProgress === "number") {
        ...
        for (let i = 1; i < totalSlides; i++) {
          ...
          const imageScale = getScaleForImage(transitionIndex, currentImageIndex, imageSpecificProgress);
          images.forEach((img) => {
            img.style.transform = `scale(${imageScale})`;
          });
        }
