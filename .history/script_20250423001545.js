...
// Create image containers and strips
...
ScrollTrigger.create({
    ...
    start: "top top",
    end: `+=${totalSlides * 1000}vh`, // Adjust based on the number of slides
    pin: true,
    pinSpacing: true,
    scrub: 1,
            ...
            for (let i = 1; i < totalSlides; i++) {
                ...
                if (transitionIndex < currentImageIndex) {
                    ...
                    strips.forEach((strip, stripIndex) => {
                        ...
                        strip.style.clipPath = `polygon(0% ${stripLowerBound}%, 100% ${stripLowerBound}%, 100% ${stripLowerBound - 0.1}%, 0% ${stripLowerBound - 0.1}%)`;
                    });
                }
...
function calculateImageProgress(scrollProgress) {
    const totalScrollDistance = totalSlides * 1000;
    return scrollProgress * totalSlides;
}
