...
function setupPageTransitions() {
  ...
  // 1) Attach click handler to links
  links.forEach(link => {
    link.addEventListener('click', e => {
      ...
      // 2) Slide up the current page content
      gsap.timeline({
        onComplete: () => {
          window.location.href = href; // Navigate to the new page after transition
        }
      })
      .to("body", {
        y: "-100%",          // Slide up the page
        duration: 0.8,       // Duration of the slide-out
        ease: "power2.inOut" // Smooth easing for a natural effect
      });
    });
  });

  // 3) On page load, slide in the new content and fade it in
  gsap.fromTo("body",
    { opacity: 0, y: "100%" }, // Start slightly below the screen
    { opacity: 1, y: "0%", duration: 0.8, ease: "power2.out" } // Slide up and fade in
  );
}
