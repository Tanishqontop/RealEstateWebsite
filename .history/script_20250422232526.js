...
firstSlideImg.computedStyleMap.transform = "scale(1.25)";
            ...
            onUpdate: (self) ⇒ {
                ...
                const firstSlideImgScale = getScaleForImage(
                    ...
                    const imgContainer = document.getElementById(
                        ...
                        const stripPosition FromBottom = stripsCount - stripIndex - 1;
                        const stripUpperBound =
                        stripPosition FromBottom (100 stripsCount);
const stripLowerBound =
(stripPositionFromBottom + 1) * (100 / stripsCount);
...
{currentstripUpperBound}%, 0% ${currentstripUpperBound}%)`;
    ...
    strips.forEach((strip, stripIndex) ⇒ {
        ...
        const stripLowerBound =
        (stripPosition FromBottom + 1) * (100 / stripsCount);
        strip.style.clipPath = `polygon (0% ${strip Lower Bound} %, 100% ${stripLowerBound} %, 100% ${stripLower Bound}%,
        0% ${stripLowerBound}%)`;
        });
        }
