...
function setupPageTransitions() {
  ...
  links.forEach(link => {
    link.addEventListener('click', e => {
      ...
      .to("body", {
        y: "-100%",          // Slide out the page upwards
        duration: 0.8,       // Duration of the slide
        ease: "power2.inOut" // Smooth easing
      });
    });
  });

  // 3) On page load, fade in the content (new page content will fade in)
  gsap.fromTo("body",
    { opacity: 0, y: "10%" }, // Start slightly below the screen
    { opacity: 1, y: "0%", duration: 0.8, ease: "power2.out" } // Smooth fade-in and slide-up
  );
}
